...
copy_files() {
    ...
    # 确保目录权限正确
    chown -R root:root /usr/local/frpc
    chmod -R 755 /usr/local/frpc
...
view_monitor_logs() {
    ...
    read -p "请选择操作 (1-2，直接回车返回): " action
    ...
    case "$action" in
        ...
        2)
            echo ""
            read -p "请输入要查看的服务编号 (1-10，直接回车返回): " service_num
            ...
            if [ -f "$log_file" ]; then
                ...
                echo "错误：未找到 $service_name 的监控日志文件"
            fi
            ;;
        *)
            echo "错误：无效的操作"
...
main_menu() {
    ...
    echo ""
        ...
        read -p "请选择操作 (1-8，直接回车退出): " choice
